...
2. **Desktop has no `default_route_bypasses_captive`.** Android added that gate because it has a bound-vs-unbound distinction; desktop isolates via netns at the OS level and has no per-request binding. The field is deliberately absent, not forgotten.
